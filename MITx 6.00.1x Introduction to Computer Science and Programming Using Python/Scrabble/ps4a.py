...
def getWordScore(word, n):
    ...
    # points after every letter
    basePoint=0
    for letter in word:
        basePoint+= SCRABBLE_LETTER_VALUES[letter]
    # point after every letter is used
    extraPoint=0
    if len(word)==n:
        extraPoint=50
    # points after the length
    multipPoint=len(word)
    
    return (basePoint*multipPoint)+extraPoint
...
#    inList=False
    ...
    #is the word match with the hand?
    fromHand=True
    wordDict=getFrequencyDict(word)
    HandDict=hand.copy()
    try:
        for key,value in wordDict.items():
            if value > HandDict[key]:
                fromHand=False
                break
    except:
        fromHand=False
        
    if fromHand==True and inList==True:
        return True
    else:
        return False
...
#word="rapture"
#print ("rapture: ", isValidWord(word, hand, wordList))

#word='hello'
#hand= {'e': 1, 'l': 2, 'o': 1, 'f': 1}
#print ("hello: ", isValidWord(word, hand, wordList))
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while True:
        ...
        # Otherwise (the input is not a single period):
            ...
            if isValidWord(newWord, hand, wordList)==False:
...
def playGame(wordList):
    ...
    """
#    should use the HAND_SIZE constant to determine the number of cards in a hand
